...
export default function Bonfire(props: Props) {
    ...
    const renderbonFire = useCallback((bonFireImageRef) => {
        ...
        bonFireContext?.drawImage(bonFireImageRef, column.current*frameWidth, 0, frameWidth, frameHeight, 0, 0, frameWidth, frameHeight);
    ...
    useEffect(() => {
        ...
        bonFireImageRef.current.onload = () => {
            if (!intervalId.current) {
                intervalId.current = setInterval(() => renderbonFire(bonFireImageRef.current), 100);
            }
        };
    ...
    return (
        <canvas
            ...
            id={'bonfire'} ref={bonFireRef}  height={frameHeight} width={frameWidth} />
    );
}
